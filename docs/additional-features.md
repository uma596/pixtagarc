...
| 37 | インポート：処理パターン選択UI（チェックボックス組み合わせからの置き換え） | 未実装 |
| 38 | 検索：タグが一つもついていない画像の検索 | 未実装 |
| 39 | ビューア画面：メニュー機能をすべてコンテキストメニューからも操作可能にする | 未実装 |
...
## 37. インポート：処理パターン選択UI（チェックボックス組み合わせからの置き換え）

### 概要

現在のインポートダイアログでは「サブフォルダも含める」「既存スキップ」「全データクリア」「フォルダごとに1作品」の4つのチェックボックスの組み合わせで動作が決まるが、組み合わせが直感的でない。これをラジオボタンまたはドロップダウンによる「処理パターン選択」に置き換え、ユーザーが意図する動作を1アクションで選べるようにする。

### 現状の問題

- 「サブフォルダON + フォルダ作品ON」「サブフォルダOFF + フォルダ作品ON」の違いが分かりにくい
- 「全データクリア + 既存スキップ」は排他なのにUIが排他的でない
- 初見で何をチェックすれば望みの動作になるか判断しづらい

### 処理パターン案

| パターン | 説明 | 旧チェック相当 |
|---------|------|-------------|
| 差分インポート | 未登録のファイルのみ追加（デフォルト） | サブフォルダON + 既存スキップON |
| フォルダを1作品として追加 | 選択フォルダ直下を1作品として追加 | サブフォルダOFF + フォルダ作品ON + 既存スキップON |
| サブフォルダごとに作品化 | 各サブフォルダを独立作品として追加 | サブフォルダON + フォルダ作品ON + 既存スキップON |
| 全クリア＆再インポート | DBを全削除してから全ファイルをインポート | サブフォルダON + 全クリアON |
...
- `import.fxml` の CheckBox 群を `ToggleGroup` + `RadioButton` に置き換え
- `ImportController` でパターン選択に応じてフラグを設定し `ImportService` に渡す
- `ImportService` のコンストラクタは内部的には従来のフラグ（`recursive`, `skipExisting`, `clearAll`, `groupByFolder`）で動作（リファクタリングは段階的に行う）

### 実装箇所

| ファイル | 変更内容 |
|---------|---------|
| `fxml/import.fxml` | CheckBox → RadioButton (ToggleGroup) に置き換え |
| `controller/ImportController.java` | パターン選択→フラグ変換ロジック |

### 工数・優先度

| 工数 | 優先度 | 理由 |
|------|--------|------|
| 小 | 中 | UI変更のみ。内部ロジックは変更不要。ユーザビリティ向上 |


---

## 38. 検索：タグが一つもついていない画像の検索

### 概要

タグが一つも付与されていない画像を検索するフィルター条件を追加する。インポート直後のタグ未設定画像を一括で見つけて整理する用途。

### ユースケース

- インポート後にタグ未設定の画像を洗い出してタグ付けしたい
- 「タグなし」の画像がどれくらい残っているか把握したい
- タグ付け作業の進捗管理
...
| 項目 | 内容 |
|------|------|
| UI | タグフィルター（MenuButton）に「タグなし」選択肢を追加 |
| 検索条件 | `image_tags` テーブルに該当画像のレコードが存在しない |
| 他のフィルターとの併用 | キーワード・作者・Starフィルターと組み合わせ可能 |
| 「タグなし」＋タグ選択 | 排他（「タグなし」選択時は個別タグ選択を無効化） |
...
| ファイル | 変更内容 |
|---------|---------|
| `controller/MainController.java` | タグフィルターに「タグなし」CheckMenuItem追加、検索条件構築時の分岐 |
| `dto/SearchCondition.java` | `noTags` フラグ追加 |
| `service/SearchService.java` | `noTags=true` 時に NOT EXISTS サブクエリ追加 |

### 工数・優先度

| 工数 | 優先度 | 理由 |
|------|--------|------|
| 小 | 高 | タグ管理のワークフローに必須。SQL1行の追加で実現可能 |


---

## 39. ビューア画面：メニュー機能をすべてコンテキストメニューからも操作可能にする

### 概要

ビューアのトップツールバー（情報バー）に配置されている操作（表示モード切替、綴じ方向切替、オフセット調整、ページ送り）を、すべてコンテキストメニューからもアクセス可能にする。情報バーを非表示にしたまま全操作が行える完全キーボードレス・マウスのみ運用を実現する。

### 現在コンテキストメニューにない操作

| 操作 | 現在のアクセス方法 | コンテキストメニュー |
|------|-------------------|-------------------|
| 見開き/単ページ切替 | ツールバーのToggleButton | ✅ 対応済み |
| 情報バー表示/非表示 | コンテキストメニュー | ✅ 対応済み |
| 右綴じ/左綴じ切替 | ツールバーのToggleButton | ❌ 未対応 |
| オフセット +/- | ツールバーのボタン | ❌ 未対応 |
| ページ送り（前/次） | ツールバーのボタン・クリック・スクロール | ✅ 他の方法で対応済み |
| 閉じる | ツールバーのボタン | ❌ 未対応（ウィンドウ×ボタンで代替可） |
...
右クリック →
    ...
    ─────────────────
    見開き表示              ← 既存
    右綴じ / 左綴じ         ← 新規（CheckMenuItem）
    オフセット →            ← 新規（サブメニュー: -1, 0にリセット, +1）
    情報バーを表示          ← 既存
    ─────────────────
    画像をDBから削除...
```
...
| メニュー項目 | 種類 | 動作 |
|-------------|------|------|
| 右綴じ / 左綴じ | CheckMenuItem | チェック時: 右綴じ、未チェック時: 左綴じ。切替後にupdateDisplay() |
| オフセット → -1 | MenuItem | `pageOffset--`、updateDisplay() |
| オフセット → リセット | MenuItem | `pageOffset = 0`、updateDisplay() |
| オフセット → +1 | MenuItem | `pageOffset++`、updateDisplay() |
...
| ファイル | 変更内容 |
|---------|---------|
| `controller/ViewerController.java` | `setupContextMenu()` に綴じ方向CheckMenuItem・オフセットサブメニューを追加 |

### 工数・優先度

| 工数 | 優先度 | 理由 |
|------|--------|------|
| 小 | 中 | 情報バー非表示時の操作性補完。追加するのはMenuItem数個のみ |
